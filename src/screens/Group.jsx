...
const Group = () => {
    ...
    const leaveGroup = (event) => {
        ...
        if (!selectedMember && groupInfo.group_members.filter(member => member.invitationStatus === "JOINED" && member.role !== "ADMIN").length !== 0 && admin)
    ...
    return (
        groupInfo != null ? <Container>
            <Button variant="dark" className="position-fixed speed-dial" size="lg" onClick={() => { setShowModalEmails(true) }}><i className="bi bi-plus-lg"></i></Button>
            <Row className='mt-2 mt-md-5'>
                <Col className="p-2">
                    <div className="d-flex justify-content-between">
                        ...
                        <div className="d-flex">
                            <h1><i onClick={() => setShowModalAdmin(true)} className="fa-solid fa-right-from-bracket align-middle fa-rotate-180"></i></h1>
                            ...
                            <Card className="border-0 h-100">
                                <Card.Body className="">
                                    <div className="d-flex justify-content-between align-self-center">
                                        {member.invitationStatus === "PENDING" && admin ? <div className="avatar-box d-flex position-relative align-self-center">
                                            ...
                                            <img className="rounded avatar" src={member.profilePic} alt="Avatar" style={{ width: 45, height: 45 }} />
                                        </div> : <img className="rounded avatar align-self-center" src={member.profilePic} alt="Avatar" style={{ width: 45, height: 45 }} />
                                        }
                                        ...
                                        {admin && member.id !== id && member.invitationStatus !== "PENDING" ? <NavDropdown className="align-self-center" menuVariant="dark" title={<i className="bi bi-three-dots-vertical fs-6"></i>}>
            ...
            <Modal centered show={showModalAdmin} fullscreen={false} onHide={() => {
                ...
                <Form noValidate onSubmit={(event) => { leaveGroup(event) }}>
                    ...
                    <Modal.Body>
                        ...
                        {
                            groupInfo.group_members.filter(member => member.invitationStatus === "JOINED" && member.role !== "ADMIN").length !== 0 && admin ?
                                groupInfo.group_members.map((member, index) => {
                                    if (member.invitationStatus === "JOINED" && member.role !== "ADMIN")
                                        return (
